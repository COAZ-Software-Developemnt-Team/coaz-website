...
const Menu = () => {
  ...
  return (
    <div style={{backgroundImage:'url(/images/bg_8.jpg)'}} className='relative flex flex-col space-y-4 w-full h-fit md:h-36 py-4 md:py-0 px-4 xs:px-[8%] sm:px-[13%] md:px-[5%] xl:px-[13%] font-nunito bg-center bg-cover shrink-0'>
      ...
      <div ref={menuButtonsRef} className={`hidden md:flex py-2 w-full h-16 sm:h-16 px-2 sm:px-4 items-center z-10 bg-white shrink-0 shadow-lg`}>
      ...
      <div className="relative w-full h-fit">
        <div className="relative md:hidden flex flex-row w-full h-[56px] px-4 bg-white space-x-2 rounded-full items-center justify-between z-10 overflow-hidden">
    ...
    if(!droppedDown) {
      if(menuRef.current && dropDownRef.current && menusRef.current) {
        ...
        if(parent) {
          ...
          dropDownRef.current.animate({height:[0,size.height+'px']},{duration:500,easing:'ease-in-out'}).addEventListener('finish',() => {
...
const MenuText = ({name,menus,parentRef,isCalc,mobileMode}) => {
  return (
    <div className={`flex flex-row ${parentRef || mobileMode?isCalc?'w-fit space-x-2 font-semibold text-heading':'w-full justify-between font-semibold text-heading':'font-bold w-fit space-x-2'} text-sm tracking-wider px-4 py-2 items-center shrink-0 capitalize`}>
